    ...
    case MOVING:
    {
        ...
        for (unsigned int i = 0; i < 6; i++) {
            if (!this->Servos[i]->atTargetAngle()) {
                atTargetAngle = false;
                logger.info("not at Target angle" + String(i) + ", current="+String(Servos[i]->getCurrentAngle()));
            }
        }

        if (atTargetAngle) {
            // std::cout << "/* attargetangelell */" << '\n';
            logger.info("at Target angle j1=" + String(this->Servos[0]->getCurrentEncPos())
                + " j2=" + String(this->Servos[1]->getCurrentEncPos()));
